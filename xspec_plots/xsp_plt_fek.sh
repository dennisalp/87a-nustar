...
a1.errorbar(xx, y3, xerr=xe, yerr=[y2,y4], c='tab:blue', fmt='none', lw=1.5, zorder=10, label='NuSTAR')
a1.errorbar(x2, b2, c='k', lw=1, zorder=4)

a1.errorbar(xxx, xy3, xerr=xxe, yerr=[xy2,xy4], c='tab:orange', fmt='none', lw=1.5, zorder=9, label='XMM-Newton')
a1.errorbar(xx2, xb2, fmt='--', c='k', lw=1, zorder=3)
a1.legend()

# a1.set_xscale('log')
# a1.set_yscale('log')
# a1.set_ylim([1.e-5, 2*(y3+y4).max()])
a1.set_ylim([0.7e-5, 2.5e-5])
a1.ticklabel_format(scilimits=(-3,3))
a1.set_ylabel('\$N_E\$ (s\$^{-1}\$ keV\$^{-1}\$ cm\$^{-2}\$)')
...
a2.set_xlim([5.9, 7.4])
a2.set_ylim([-3.5, 3.5])
plt.savefig('/Users/silver/box/phd/pro/87a/nus/art/fig/fek.pdf', bbox_inches='tight', pad_inches=0.1, dpi=300)
"
