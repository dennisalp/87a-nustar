...
def plt_tau(ax1, d2, p2, pp, hh, ll):
    ...
    legend = ax2.legend(hh[:8], ll[:8], handlelength=0.6, framealpha=0.8, loc='upper right').set_zorder(20002)
...
def plt_plasma(logs):
    ...
    for ii, pp in enumerate(logs):
        ...
        if ii == 8: plt.plot(1,1); plt.plot(1,1)
        plt.semilogx(ts, em/1e58, label=tmp, ls='-' if ii < 8 else '--')
...
def plt_plasma_cal(logs):
    ...
    for ii, pp in enumerate(logs):
        ...
        if ii == 4:
            for ii in range(0,6):
                plt.plot(1,1)
        plt.semilogx(ts, em/1e58, label=tmp, ls='-' if ii < 4 else '--')
    ...
    ax1.set_zorder(1)  # default zorder is 0 for ax1 and ax2
    ax1.patch.set_visible(False)  # prevents ax1 from hiding ax2
    ax1.legend(hh[:4], ll[:4], handlelength=0.6, framealpha=0.8, loc='upper right').set_zorder(20002)
...
# this is the one with xmm/pn
logs = sorted(glob('/Users/silver/box/phd/pro/87a/nus/xsp/log/dos_em10_011_pl0_g?.txt'))
plt.figure(figsize=(5, 3.75))
plt_plasma(logs)

out = '/Users/silver/box/phd/pro/87a/nus/art/fig/dos_v2_pn.pdf'
...
out = '/Users/silver/box/phd/pro/87a/nus/art/fig/dos_v2_no_pn.pdf'
plt.savefig(out, bbox_inches='tight', pad_inches=0.1, dpi=300)


logs = sorted(glob('/Users/silver/box/phd/pro/87a/nus/xsp/log/dos_em10_011_pl0_g?.txt'))
logs = logs + sorted(glob('/Users/silver/box/phd/pro/87a/nus/xsp/log/dos_em10_013_pl0_g?.txt'))
plt.figure(figsize=(5, 3.75))
plt_plasma(logs)
out = '/Users/silver/box/phd/pro/87a/nus/art/fig/dos_v2.pdf'
plt.savefig(out, bbox_inches='tight', pad_inches=0.1, dpi=300)
...
l1 = sorted(glob('/Users/silver/box/phd/pro/87a/nus/xsp/log/dos_em10_013_pl0_g?.txt'))
l2 = sorted(glob('/Users/silver/box/phd/pro/87a/nus/xsp/log/dos_em10_011_pl0_g?.txt'))
logs = [l1[0], l1[2], l1[4], l1[7], l2[0], l2[2], l2[4], l2[7]]
tt = [tt[0], tt[2], tt[4], tt[7], tt[0], tt[2], tt[4], tt[7]]

plt_plasma_cal(logs)
out = '/Users/silver/box/phd/pro/87a/nus/art/fig/dos_cal_v2.pdf'
plt.savefig(out, bbox_inches='tight', pad_inches=0.1, dpi=300)
